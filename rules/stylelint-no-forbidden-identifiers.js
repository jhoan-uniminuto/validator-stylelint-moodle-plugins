...
const plugin = stylelint.createPlugin(ruleName, function(primaryOption, secondaryOptions) {
    ...
    const forbiddenIdsPath = secondaryOptions.filePath;
    let forbiddenIdsRaw = [];
    
    try {
        forbiddenIdsRaw = require(forbiddenIdsPath);
    } catch (e) {
        console.error(`[${ruleName}] Error al cargar el archivo de clases prohibidas en: ${forbiddenIdsPath}`, e);
        return () => {}; 
    }
